        ...
        if (status == 2) {
            ...
            if (menu_detectplane) {
                ...
                if (pPlane) {
                    ...
                    cout << "No plane detected. Point the camera to a planar "
                            "region."
                         << endl;
                }
...
void ViewerAR::SetImagePose(const cv::Mat &im,
                            const cv::Mat &Tcw,
                            const int &status,
...
void ViewerAR::GetImagePose(cv::Mat &im,
                            cv::Mat &Tcw,
                            int &status,
...
void ViewerAR::PrintStatus(const int &status,
                           const bool &bLocMode,
                           cv::Mat &im) {
    if (!bLocMode) {
        switch (status) {
...
void ViewerAR::DrawCube(const float &size,
                        const float x,
                        const float y,
                        const float z) {
    pangolin::OpenGlMatrix M =
