...
class Tracking {
 public:
    ...
    cv::Mat GrabImageMonocular(const cv::Mat &im,
                               const double &timestamp,
                               string filename);
    // cv::Mat GrabImageImuMonocular(const cv::Mat &im, const double
    // &timestamp);
 ...
 protected:
    ...
    void ComputeGyroBias(const vector<Frame *> &vpFs,
                         float &bwx,
                         float &bwy,
                         float &bwz);
    void ComputeVelocitiesAccBias(const vector<Frame *> &vpFs,
                                  float &bax,
                                  float &bay,
                                  float &baz);
    ...
    // no matches to
    // points in the map. Still tracking will continue if there are enough
    // matches with temporal points.
