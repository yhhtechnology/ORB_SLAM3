...
Eigen::Matrix<double, 3, 3> ExpSO3(const double &x,
                                   const double &y,
                                   const double &z) {
...
cv::Mat InverseRightJacobianSO3(const float &x,
                                const float &y,
                                const float &z) {
    cv::Mat I = cv::Mat::eye(3, 3, CV_32F);
    const float d2 = x * x + y * y + z * z;
...
void Preintegrated::IntegrateNewMeasurement(const cv::Point3f &acceleration,
    ...
    // Velocity is updated secondly, as it depends on previously computed
    // rotation.
...
cv::Mat Preintegrated::GetDeltaRotation(const Bias &b_) {
    std::unique_lock<std::mutex> lock(mMutex);
    cv::Mat dbg = (cv::Mat_<float>(3, 1) << b_.bwx - b.bwx, b_.bwy - b.bwy,
                   b_.bwz - b.bwz);
...
cv::Mat Preintegrated::GetDeltaVelocity(const Bias &b_) {
    ...
    cv::Mat dbg = (cv::Mat_<float>(3, 1) << b_.bwx - b.bwx, b_.bwy - b.bwy,
                   b_.bwz - b.bwz);
    cv::Mat dba = (cv::Mat_<float>(3, 1) << b_.bax - b.bax, b_.bay - b.bay,
                   b_.baz - b.baz);
