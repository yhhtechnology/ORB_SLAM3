            ...
            if (!vIndices.empty()) {
                ...
                for (vector<size_t>::const_iterator vit = vIndices.begin(),
                    ...
                    if (F.Nleft == -1 && F.mvuRight[idx] > 0) {
                        const float er =
                            fabs(pMP->mTrackProjXR - F.mvuRight[idx]);
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    if (bestLevel != bestLevel2 ||
                        ...
                        if (F.Nleft != -1 &&
                            F.mvLeftToRightMatch[bestIdx] !=
                                -1) {  // Also match with the stereo observation
                                       // at right camera
                            F.mvpMapPoints[F.mvLeftToRightMatch[bestIdx] +
                                           F.Nleft] = pMP;
                            nmatches++;
                            right++;
                        }
        ...
        if (F.Nleft != -1 && pMP->mbTrackInViewR) {
            ...
            if (nPredictedLevel != -1) {
                ...
                if (bestDist <= TH_HIGH) {
                    ...
                    if (F.Nleft != -1 &&
                        F.mvRightToLeftMatch[bestIdx] !=
                            -1) {  // Also match with the stereo observation at
                                   // right camera
...
int ORBmatcher::SearchByProjection(KeyFrame *pKF,
    ...
    for (int iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++) {
        ...
        const vector<size_t> vIndices =
            pKF->GetFeaturesInArea(uv.x, uv.y, radius);
...
int ORBmatcher::SearchForTriangulation(
    ...
    if (!pKF1->mpCamera2 && !pKF2->mpCamera2) {
        ...
        tll = pKF1->GetRotation() *
              ...
              pKF1->GetTranslation();
        tlr = pKF1->GetRotation() * (-pKF2->GetRightRotation().t() *
                                     pKF2->GetRightTranslation()) +
              pKF1->GetTranslation();
        trl = pKF1->GetRightRotation() *
                  (-pKF2->GetRotation().t() * pKF2->GetTranslation()) +
              pKF1->GetRightTranslation();
        trr = pKF1->GetRightRotation() * (-pKF2->GetRightRotation().t() *
                                          pKF2->GetRightTranslation()) +
              pKF1->GetRightTranslation();
    }
    ...
    while (f1it != f1end && f2it != f2end) {
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                const bool bStereo1 =
                    (!pKF1->mpCamera2 && pKF1->mvuRight[idx1] >= 0);
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2;
                    ...
                    const bool bStereo2 =
                        (!pKF2->mpCamera2 && pKF2->mvuRight[idx2] >= 0);
                    ...
                    if (pCamera1->epipolarConstrain(
                            pCamera2, kp1, kp2, R12, t12,
...
int ORBmatcher::SearchForTriangulation_(
    KeyFrame *pKF1,
    KeyFrame *pKF2,
    cv::Matx33f F12,
    vector<pair<size_t, size_t> > &vMatchedPairs,
    ...
    if (!pKF1->mpCamera2 && !pKF2->mpCamera2) {
        ...
        tll = pKF1->GetRotation_() *
                  (-pKF2->GetRotation_().t() * pKF2->GetTranslation_()) +
              pKF1->GetTranslation_();
        tlr = pKF1->GetRotation_() * (-pKF2->GetRightRotation_().t() *
                                      pKF2->GetRightTranslation_()) +
              pKF1->GetTranslation_();
        trl = pKF1->GetRightRotation_() *
                  (-pKF2->GetRotation_().t() * pKF2->GetTranslation_()) +
              pKF1->GetRightTranslation_();
        trr = pKF1->GetRightRotation_() * (-pKF2->GetRightRotation_().t() *
                                           pKF2->GetRightTranslation_()) +
              pKF1->GetRightTranslation_();
    }
        ...
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2;
                    ...
                    if (pCamera1->epipolarConstrain_(
                            pCamera2, kp1, kp2, R12, t12,
                            pKF1->mvLevelSigma2[kp1.octave],
                            pKF2->mvLevelSigma2[kp2.octave]) ||
                        bCoarse)  // MODIFICATION_2
        ...
        if (f1it->first == f2it->first) {
            for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2;
                    ...
                    cv::Mat x3D;
                    if (pCamera1->matchAndtriangulate(
                            kp1, kp2, pCamera2, Tcw1, Tcw2,
                            pKF1->mvLevelSigma2[kp1.octave],
            ...
            if (!LastFrame.mvbOutlier[i]) {
                ...
                if (bForward)
                    vIndices2 = CurrentFrame.GetFeaturesInArea(
                        uv.x, uv.y, radius, nLastOctave);
                else if (bBackward)
                    vIndices2 = CurrentFrame.GetFeaturesInArea(
                        uv.x, uv.y, radius, 0, nLastOctave);
                else
                    vIndices2 = CurrentFrame.GetFeaturesInArea(
                ...
                if (CurrentFrame.Nleft != -1) {
                    ...
                    // Search in a window. Size depends on scale
                    float radius =
                        th * CurrentFrame.mvScaleFactors[nLastOctave];

                    vector<size_t> vIndices2;

                    if (bForward)
                        vIndices2 = CurrentFrame.GetFeaturesInArea(
                            uv.x, uv.y, radius, nLastOctave, -1, true);
                    else if (bBackward)
                        vIndices2 = CurrentFrame.GetFeaturesInArea(
                            uv.x, uv.y, radius, 0, nLastOctave, true);
                    else
                        vIndices2 = CurrentFrame.GetFeaturesInArea(
                            uv.x, uv.y, radius, nLastOctave - 1,
                            nLastOctave + 1, true);
                    ...
                    for (vector<size_t>::const_iterator vit = vIndices2.begin(),
                        ...
                        if (CurrentFrame.mvpMapPoints[i2 + CurrentFrame.Nleft])
                            if (CurrentFrame
                                    .mvpMapPoints[i2 + CurrentFrame.Nleft]
                                    ->Observations() > 0)
                                continue;

                        const cv::Mat &d = CurrentFrame.mDescriptors.row(
                    ...
                    if (bestDist <= TH_HIGH) {
                        CurrentFrame
                            .mvpMapPoints[bestIdx2 + CurrentFrame.Nleft] = pMP;
                        nmatches++;
                        if (mbCheckOrientation) {
                            cv::KeyPoint kpLF =
                                (LastFrame.Nleft == -1)
                            ...
                            cv::KeyPoint kpCF =
                                CurrentFrame.mvKeysRight[bestIdx2];
        ...
        if (pMP) {
            if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
                ...
                const vector<size_t> vIndices2 = CurrentFrame.GetFeaturesInArea(
                    uv.x, uv.y, radius, nPredictedLevel - 1,
                    nPredictedLevel + 1);
