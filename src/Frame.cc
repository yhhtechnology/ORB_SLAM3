...
void Frame::SetImuPoseVelocity(const cv::Mat &Rwb,
                               const cv::Mat &twb,
                               const cv::Mat &Vwb) {
    mVw = Vwb.clone();
    cv::Mat Rbw = Rwb.t();
...
void Frame::UpdatePoseMatrices() {
    ...
    mtcwx =
        cv::Matx31f(mtcw.at<float>(0), mtcw.at<float>(1), mtcw.at<float>(2));
}
...
cv::Mat Frame::GetImuPose() {
    ...
    Twb.rowRange(0, 3).col(3) =
        mRwc * mImuCalib.Tcb.rowRange(0, 3).col(3) + mOw;
    return Twb.clone();
}
...
// ORB extraction
    ...
    mTrlx = cv::Matx34f(
        Rrl.at<float>(0, 0), Rrl.at<float>(0, 1), Rrl.at<float>(0, 2),
        ...
        if ((*it).size() >= 2 && (*it)[0].distance < (*it)[1].distance * 0.7) {
            ...
            float depth =
                static_cast<KannalaBrandt8 *>(mpCamera)->TriangulateMatches(
                    ...
                    mvKeysRight[(*it)[0].trainIdx + monoRight], mRlr, mtlr,
                    sigma1, sigma2, p3D);
            if (depth > 0.0001f) {
...
bool Frame::isInFrustumChecks(MapPoint *pMP,
