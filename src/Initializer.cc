...
bool Initializer::Initialize(const Frame &CurrentFrame,
    ...
    // (0.40-0.45)
    if (RH > 0.40) {
        return ReconstructH(vbMatchesInliersH, H, K, R21, t21, vP3D,
                            vbTriangulated, minParallax, 50);
    } else {
        return ReconstructF(vbMatchesInliersF, F, K, R21, t21, vP3D,
                            vbTriangulated, minParallax, 50);
    }
...
void Initializer::DecomposeE(const cv::Mat &E,
                             cv::Mat &R1,
                             cv::Mat &R2,
                             cv::Mat &t) {
    cv::Mat u, w, vt;
