...
void Optimizer::FullInertialBA(Map* pMap,
        ...
        if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
            ...
            if (pKFi->bImu && pKFi->mPrevKF->bImu) {
                ...
                if (!bInit) {
                    ...
                    egr->setVertex(1, VG2);
                    cv::Mat cvInfoG =
                        pKFi->mpImuPreintegrated->C.rowRange(9, 12)
                            .colRange(9, 12)
                            .inv(cv::DECOMP_SVD);
                    ...
                    ear->setVertex(1, VA2);
                    cv::Mat cvInfoA =
                        pKFi->mpImuPreintegrated->C.rowRange(12, 15)
                            .colRange(12, 15)
                            .inv(cv::DECOMP_SVD);
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        if (nLoopId == 0) {
            cv::Mat Tcw = Converter::toCvSE3(VP->estimate().Rcw[0],
                                             VP->estimate().tcw[0]);
            pKFi->SetPose(Tcw);
        } else {
    ...
    for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(),
        ...
        for (map<KeyFrame*, tuple<int, int>>::const_iterator
            ...
            if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
                ...
                // Monocular observation
                if (leftIndex != -1 &&
                    pKFi->mvuRight[get<0>(mit->second)] < 0) {
...
void Optimizer::OptimizeEssentialGraph(KeyFrame* pCurKF,
    ...
    for (MapPoint* pMPi : vpNonCorrectedMPs) {
        ...
        Eigen::Matrix<double, 3, 3> RNonCorrectedwr = Converter::toMatrix3d(
            TNonCorrectedwr.rowRange(0, 3).colRange(0, 3));
...
void Optimizer::LocalInertialBA(KeyFrame* pKF,
    ...
    // 最大地图数量: Nd
    // 这里可能会有BUG!!!!, 如果pCurrentMap->KeyFramesInMap()数量小于2呢;
    // 或者如果
    const int Nd = std::min((int)pCurrentMap->KeyFramesInMap() - 2, maxOpt);
    const unsigned long maxKFid = pKF->mnId;
    // vpOptimizableKFs : 当前关键帧 + 关键帧的mPrevKF + ....
    vector<KeyFrame*> vpOptimizableKFs;
    // vpNeighsKFs : 当前关键帧的共视关键帧 (获取的是 mvpOrderedConnectedKeyFrames )
        // mvpOrderedConnectedKeyFrames : 是根据共视程度排序的关键帧, 由大到小
    // maxCovKF : hardcode设定为0, 相当于 ORBSLAM3是没有用到共视关键帧做LBA的
    ...
    // lFixedKeyFrames : 固定的帧(1. vpOptimizableKFs 队列中最后一个固定住. 2. 共视地图点的观测关键帧)
    // 这里设置了最大的固定上限为 200
...
Eigen::MatrixXd Optimizer::Condition(const Eigen::MatrixXd& H,
                                     const int& start,
                                     const int& end) {
    ...
    for (unsigned int i = 0; i < vpMPs.size(); ++i) {
        ...
        for (map<KeyFrame*, tuple<int, int>>::const_iterator mit =
            ...
            if (spKeyFrameBA.find(pKF) == spKeyFrameBA.end() || pKF->isBad() ||
                pKF->mnId > maxKFid ||
                pKF->mnBALocalForKF != pCurrentKF->mnId ||
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,
    ...
    if (!vToErase.empty()) {
        ...
        Verbose::PrintMess(
            "LBA: There are " + to_string(vToErase.size()) +
        ...
        for (MapPoint* pMPi : spErasedMPs) {
            if (pMPi->isBad()) {
                Verbose::PrintMess("LBA: MP " + to_string(pMPi->mnId) +
                                       " has lost almost all the observations, "
                                       "its origin map is " +
    ...
    for (KeyFrame* pKFi : vpAdjustKF) {
        ...
        if (numMonoOptPoints + numStereoOptPoints < 50) {
            Verbose::PrintMess(
                "LBA ERROR: KF " + to_string(pKFi->mnId) + " has only " +
                    to_string(numMonoOptPoints) + " monocular and " +
                    to_string(numStereoOptPoints) + " stereo points",
                Verbose::VERBOSITY_DEBUG);
        }
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF,
    ...
    for (vector<KeyFrame*>::iterator lit = vNeighCurr.begin(),
                                     lend = vNeighCurr.end();
         lit != lend; lit++) {
        if (i >= maxCovKF) break;
        KeyFrame* pKFi = *lit;
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame* pFrame, bool bRecInit) {
    ...
    ear->setVertex(0, VAk);
    ear->setVertex(1, VA);
    cv::Mat cvInfoA = pFrame->mpImuPreintegratedFrame->C.rowRange(12, 15)
                          .colRange(12, 15)
                          .inv(cv::DECOMP_SVD);
    Eigen::Matrix3d InfoA;
    for (int r = 0; r < 3; r++)
