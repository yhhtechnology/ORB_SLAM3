...
void Tracking::TrackStats2File() {
    ...
    for (int i = 0; i < vnKeyFramesLM.size(); ++i) {
        f << vnKeyFramesLM[i] << "," << vnMapPointsLM[i] << ","
          << vdUpdatedLM_ms[i] << "," << vdSearchLP_ms[i] << ","
          << vdPoseOpt_ms[i] << endl;
    }
...
void Tracking::PrintTimeStats() {
    ...
    std::cout << "KFs in map: "
              << mpAtlas->GetAllMaps()[0]->GetAllKeyFrames().size()
              << std::endl;
    std::cout << "MPs in map: "
              << mpAtlas->GetAllMaps()[0]->GetAllMapPoints().size()
              << std::endl;
    f << "---------------------------" << std::endl;
...
void Tracking::ComputeGyroBias(const vector<Frame *> &vpFs,
                               float &bwx,
                               float &bwy,
                               float &bwz) {
    ...
    for (int i = 1; i < N; i++) {
        ...
        cv::Mat E =
            pF2->mpImuPreintegratedFrame->GetUpdatedDeltaRotation().t() *
            VisionR;
...
void Tracking::ComputeVelocitiesAccBias(const vector<Frame *> &vpFs,
                                        float &bax,
                                        float &bay,
                                        float &baz) {
    const int N = vpFs.size();
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        J.rowRange(6 * i + 3, 6 * i + 6)
            .colRange(3 * (i + 1), 3 * (i + 1) + 3) +=
            cv::Mat::eye(3, 3, CV_32F);
        J.rowRange(6 * i + 3, 6 * i + 6).colRange(3 * N, 3 * N + 3) -=
            Rwb1 * JV12;
...
void Tracking::Track() {
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        if (!mbOnlyTracking) {
            ...
            if (mState == OK) {
                ...
                if ((mVelocity.empty() && !pCurrentMap->isImuInitialized()) ||
                    ...
                    // Verbose::PrintMess("TRACK: Track with respect to the
                    // reference KF ", Verbose::VERBOSITY_DEBUG);
                ...
                if (!bOK) {
                    if (mCurrentFrame.mnId <=
                        ...
                        cout << "KF in map: " << pCurrentMap->KeyFramesInMap()
                             << endl;
        ...
        if (!mbOnlyTracking) {
            ...
            // map. We cannot retrieve
            // a local map and therefore we do not perform TrackLocalMap(). Once
            // the system relocalizes
        ...
        if (bOK || mState == RECENTLY_LOST) {
            ...
            // Huber Function)
            // pass to the new keyframe, so that bundle adjustment will finally
            // decide
            // if they are outliers or not. We don't want next frame to estimate
            // its position
            // with those points so we discard them in the frame. Only has
            // effect if lastframe is tracked
            for (int i = 0; i < mCurrentFrame.N; i++) {
                if (mCurrentFrame.mvpMapPoints[i] &&
                    mCurrentFrame.mvbOutlier[i])
                    mCurrentFrame.mvpMapPoints[i] =
...
void Tracking::StereoInitialization() {
    if (mCurrentFrame.N > 500) {
        ...
        if (mSensor == System::IMU_STEREO) {
            cv::Mat Rwb0 = mCurrentFrame.mImuCalib.Tcb.rowRange(0, 3)
                               .colRange(0, 3)
                               .clone();
            cv::Mat twb0 =
                mCurrentFrame.mImuCalib.Tcb.rowRange(0, 3).col(3).clone();
...
void Tracking::CreateMapInAtlas() {
    ...
    mnLastRelocFrameId = mnLastInitFrameId;  // The last relocation KF_id is the
                                             // current id, because it is the
                                             // new starting point for new map
    Verbose::PrintMess(
...
bool Tracking::TrackLocalMap() {
    ...
    // yhh
    {
        ...
        if(tracking_ba_callback_) {
            ...
            //     <<"\n"
            //     ;
            // printf("tracking_ba_callback_  NOT NULL \n");
...
void Tracking::CreateNewMapPoints() {
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        matcher.SearchForTriangulation(mpLastKeyFrame, pKF2, F12,
        ...
        for (int ikp = 0; ikp < nmatches; ikp++) {
            ...
            if (bStereo1)
                cosParallaxStereo1 =
                    cos(2 * atan2(mpLastKeyFrame->mb / 2,
                                  mpLastKeyFrame->mvDepth[idx1]));
            else if (bStereo2)
                cosParallaxStereo2 =
            ...
            const float &sigmaSquare1 =
                mpLastKeyFrame->mvLevelSigma2[kp1.octave];
            ...
            const float ratioOctave =
                mpLastKeyFrame->mvScaleFactors[kp1.octave] /
