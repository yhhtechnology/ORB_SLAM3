...
void LocalMapping::CreateNewMapPoints() {
    ...
    for (size_t i = 0; i < vpNeighKFs.size(); i++) {
        ...
        matcher.SearchForTriangulation_(mpCurrentKeyFrame, pKF2, F12,
        ...
        for (int ikp = 0; ikp < nmatches; ikp++) {
            ...
            cv::Mat x3D_(x3D);
            MapPoint *pMP =
                new MapPoint(x3D_, mpCurrentKeyFrame, mpAtlas->GetCurrentMap());
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA) {
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized()) {
        ...
        for (vector<KeyFrame *>::iterator itKF = vpKF.begin();
            ...
            cv::Mat _vel = ((*itKF)->GetImuPosition() -
    ...
    if ((fabs(mScale - 1.f) > 0.00001) || !mbMonocular) {
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(
            Converter::toCvMat(mRwg).t(), mScale, true);
