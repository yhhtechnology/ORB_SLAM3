...
void LoopClosing::Run() {
    ...
    while (1) {
        ...
        if (CheckNewKeyFrames()) {
            ...
            timeDetectBoW = 0;
            std::chrono::steady_clock::time_point time_StartDetectBoW =
                std::chrono::steady_clock::now();
#endif
            bool bDetected = NewDetectCommonRegions();
#ifdef REGISTER_TIMES
            std::chrono::steady_clock::time_point time_EndDetectBoW =
                std::chrono::steady_clock::now();
            double timeDetect = std::chrono::duration_cast<
            ...
            if (bDetected) {
                if (mbMergeDetected) {
                    if ((mpTracker->mSensor == System::IMU_MONOCULAR ||
                        ...
                        Verbose::PrintMess("*Merged detected",
                                           Verbose::VERBOSITY_QUIET);
                        Verbose::PrintMess(
                            "Number of KFs in the current map: " +
                                to_string(
                                    mpCurrentKF->GetMap()->KeyFramesInMap()),
                            Verbose::VERBOSITY_DEBUG);
                        cv::Mat mTmw = mpMergeMatchedKF->GetPose();
                        g2o::Sim3 gSmw2(
                            Converter::toMatrix3d(
                                mTmw.rowRange(0, 3).colRange(0, 3)),
                            Converter::toVector3d(mTmw.rowRange(0, 3).col(3)),
                            1.0);
                        cv::Mat mTcw = mpCurrentKF->GetPose();
                        g2o::Sim3 gScw1(
                            Converter::toMatrix3d(
                                mTcw.rowRange(0, 3).colRange(0, 3)),
                            Converter::toVector3d(mTcw.rowRange(0, 3).col(3)),
                            1.0);
...
bool LoopClosing::NewDetectCommonRegions() {
    ...
    if (mnLoopNumCoincidences > 0) {
        ...
        g2o::Sim3 gScl(
            Converter::toMatrix3d(mTcl.rowRange(0, 3).colRange(0, 3)),
    ...
    if (!bMergeDetectedInKF || !bLoopDetectedInKF) {
        ...
        std::chrono::steady_clock::time_point time_EndDetectBoW =
            std::chrono::steady_clock::now();
        timeDetectBoW = std::chrono::duration_cast<
                            std::chrono::duration<double, std::milli> >(
                            time_EndDetectBoW - time_StartDetectBoW)
                            .count();
#endif
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(
    ...
    if (nNumProjMatches >= nProjMatches) {
        ...
        g2o::Sim3 gSwm(
            Converter::toMatrix3d(mTwm.rowRange(0, 3).colRange(0, 3)),
    ...
    for (KeyFrame* pKFi : vpBowCand) {
            ...
            numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
        {
            ...
            if (bConverge) {
                ...
                g2o::Sim3 gScm(
                    Converter::toMatrix3d(solver.GetEstimatedRotation()),
                ...
                if (numProjMatches >= nProjMatches) {
                    ...
                    if (numOptMatches >= nSim3Inliers) {
                        g2o::Sim3 gSmw(Converter::toMatrix3d(
                                           pMostBoWMatchesKF->GetRotation()),
                                       Converter::toVector3d(
                                           pMostBoWMatchesKF->GetTranslation()),
                                       1.0);
                        g2o::Sim3 gScw = gScm * gSmw;  // Similarity matrix of
                                                       // current from the world
                                                       // position
                        cv::Mat mScw = Converter::toCvMat(gScw);
                        ...
                        if (numProjOptMatches >= nProjOptMatches) {
                            ...
                            while (nNumKFs < 3 && j < vpCurrentCovKFs.size()) {
                                ...
                                cv::Mat mTjc = pKFj->GetPose() *
                                               mpCurrentKF->GetPoseInverse();
                                g2o::Sim3 gSjc(
                                    Converter::toMatrix3d(
                                        mTjc.rowRange(0, 3).colRange(0, 3)),
                                    Converter::toVector3d(
                                        mTjc.rowRange(0, 3).col(3)),
                                    1.0);
...
void LoopClosing::MergeLocal() {
    Verbose::PrintMess("MERGE: Merge Visual detected!!!!",
                       Verbose::VERBOSITY_NORMAL);
    ...
    Verbose::PrintMess("MERGE: Check Full Bundle Adjustment",
                       Verbose::VERBOSITY_DEBUG);
    // If a Global Bundle Adjustment is running, abort it
    if (isRunningGBA()) {
        unique_lock<mutex> lock(mMutexGBA);
    ...
    Verbose::PrintMess("MERGE: Request Stop Local Mapping",
                       Verbose::VERBOSITY_DEBUG);
    mpLocalMapper->RequestStop();
    // Wait until Local Mapping has effectively stopped
    while (!mpLocalMapper->isStopped()) {
    ...
    // and MPs from the current map.
    // Later, the elements of the current map will be transform to the new
    // active map reference, in order to keep real time tracking
    ...
    while (pNewChild) {
        pNewChild->EraseChild(pNewParent);  // We remove the relation between
                                            // the old parent and the new for
                                            // avoid loop
    ...
    if (vpCurrentMapKFs.size() == 0) {
        Verbose::PrintMess(
            "MERGE: There are not KFs outside of the welding area",
            Verbose::VERBOSITY_DEBUG);
    } else {
        Verbose::PrintMess(
            "MERGE: Calculate the new position of the elements outside of the "
            "window",
            Verbose::VERBOSITY_DEBUG);
        // Apply the transformation
        {
            if (mpTracker->mSensor == System::MONOCULAR) {
                ...
                for (KeyFrame* pKFi : vpCurrentMapKFs) {
                    ...
                    if (pCurrentMap->isImuInitialized()) {
                        Eigen::Matrix3d Rcor = eigR.transpose() *
                                               vNonCorrectedSim3[pKFi]
                                                   .rotation()
                                                   .toRotationMatrix();
                        pKFi->SetVelocity(Converter::toCvMat(Rcor) *
                                          pKFi->GetVelocity());  // TODO: should
                                                                 // add here
                                                                 // scale s
                    }
                }
                for (MapPoint* pMPi : vpCurrentMapMPs) {
                    ...
                    KeyFrame* pKFref = pMPi->GetReferenceKeyFrame();
                    g2o::Sim3 g2oCorrectedSwi =
                        vCorrectedSim3[pKFref].inverse();
                    g2o::Sim3 g2oNonCorrectedSiw = vNonCorrectedSim3[pKFref];
                    ...
                    cv::Mat cvCorrectedP3Dw =
                        Converter::toCvMat(eigCorrectedP3Dw);
                    pMPi->SetWorldPos(cvCorrectedP3Dw);
...

        {
            ...
            unique_lock<mutex> mergeLock(
                pMergeMap->mMutexMapUpdate);  // We remove the Kfs and MPs in
                                              // the merged area from the old
                                              // map
...
void LoopClosing::MergeLocal2() {
...

    {
        ...
        cv::Mat R_on =
            Converter::toCvMat(mSold_new.rotation().toRotationMatrix());
        ...
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(R_on, s_on, bScaleVel,
                                                      t_on);
    ...
    vector<MapPoint*> vpCheckFuseMapPoint;  // MapPoint vector from current map
                                            // to allow to fuse duplicated
                                            // points with the old map (merge)
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap,
...
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_StartMapUpdate =
        std::chrono::steady_clock::now();

    double timeFullGBA =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
            time_StartMapUpdate - time_StartFGBA)
            .count();
    ...
    // keyframes
    // not included in the Global BA and they are not consistent with the
    // updated map.
            ...
            while (!lpKFtoCheck.empty()) {
                ...
                for (set<KeyFrame*>::const_iterator sit = sChilds.begin();
                    ...
                    if (pChild->mnBAGlobalForKF != nLoopKF) {
                        ...
                        cv::Mat Rcor =
                            pChild->mTcwGBA.rowRange(0, 3).colRange(0, 3).t() *
                            pChild->GetRotation();
...
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndMapUpdate =
        std::chrono::steady_clock::now();

    double timeMapUpdate =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
            time_EndMapUpdate - time_StartMapUpdate)
            .count();
    vTimeMapUpdate_ms.push_back(timeMapUpdate);

    double timeGBA =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
            time_EndMapUpdate - time_StartFGBA)
            .count();
