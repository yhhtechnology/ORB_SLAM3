...
void Map::AddKeyFrame(KeyFrame* pKF) {
    ...
    if (mspKeyFrames.empty()) {
        cout << "First KF:" << pKF->mnId << "; Map init KF:" << mnInitKFid
             << endl;
        mnInitKFid = pKF->mnId;
        mpKFinitial = pKF;
        mpKFlowerID = pKF;
...
void Map::RotateMap(const cv::Mat& R) {
    ...
    for (set<KeyFrame*>::iterator sit = mspKeyFrames.begin();
        ...
        Tcy.rowRange(0, 3).colRange(0, 3) =
            Tyc.rowRange(0, 3).colRange(0, 3).t();
...
void Map::ApplyScaledRotation(const cv::Mat& R,
                              const float s,
...
void Map::PrintEssentialGraph() {
    ...
    if (pFirstKF->GetParent()) {
        cout << "First KF in the essential graph has a parent, which is not "
                "possible"
             << endl;
    }
    ...
    for (int i = 0; i < vpChilds.size() && count <= (mspKeyFrames.size() + 10);
         ++i) {
        count++;
    ...
    cout << "------------------" << endl
         << "End of the essential graph" << endl;
}
